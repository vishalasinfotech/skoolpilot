                ...
                <tbody>
                    @forelse($holidays as $index => $holiday)
                        <tr wire:key="holiday-{{ $holiday->id }}">
                            <td>
                                <input type="checkbox" wire:model.live="selectedRows" value="{{ $holiday->id }}" class="form-check-input">
                            </td>
                            <td>{{ str_pad($holidays->firstItem() + $index, 2, '0', STR_PAD_LEFT) }}</td>
                            <td>
                                <span class="fw-medium">{{ $holiday->name }}</span>
                            </td>
                            <td>{{ $holiday->date ? $holiday->date->format('M d, Y') : 'N/A' }}</td>
                            <td>{{ $holiday->description ?? 'N/A' }}</td>
                            <td>{{ $holiday->school->name ?? 'N/A' }}</td>
                            <td>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a class="dropdown-item" href="{{ route('school-admin.holiday.show', $holiday->id) }}">
                                                <i class="ri-eye-fill align-bottom me-2 text-muted"></i> View
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="{{ route('school-admin.holiday.edit', $holiday->id) }}">
                                                <i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit
                                            </a>
                                        </li>
                                        <li class="dropdown-divider"></li>
                                        <li>
                                            <button class="dropdown-item text-danger" wire:click="openDeleteModal({{ $holiday->id }}, '{{ $holiday->name }}')">
                        ...
                        <tr>
                            <td colspan="8" class="text-center py-4">
                                <div class="text-muted">
                                    ...
                                    <p class="mt-2 mb-0">No holidays found</p>
                                    @if($search)

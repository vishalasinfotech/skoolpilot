                ...
                <tbody>
                    @forelse($subjects as $index => $subject)
                        <tr wire:key="subject-{{ $subject->id }}">
                            <td>
                                <input type="checkbox" wire:model.live="selectedRows" value="{{ $subject->id }}" class="form-check-input">
                            </td>
                            <td>{{ str_pad($subjects->firstItem() + $index, 2, '0', STR_PAD_LEFT) }}</td>
                            <td>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a class="dropdown-item" href="{{ route('school-admin.subject.show', $subject->id) }}">
                                                <i class="ri-eye-fill align-bottom me-2 text-muted"></i> View
                                            </a>
                                        </li>
                                        <li>
                                            <a class="dropdown-item" href="{{ route('school-admin.subject.edit', $subject->id) }}">
                                                <i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit
                                            </a>
                                        </li>
                                        <li class="dropdown-divider"></li>
                                        <li>
                                            <button class="dropdown-item text-danger" wire:click="openDeleteModal({{ $subject->id }}, '{{ $subject->name }}')">
